...
public class HUSP_SP_Contrast_MRSU {
...
//    protected String output;
    ...
    protected class LastId {
        ...
        public int maxTrsuInT;

        public LastId(int swu, ULinkList uLinkList, int maxTrsuInT) {
            this.swu = swu;
            this.uLinkList = uLinkList;
            this.maxTrsuInT = maxTrsuInT;
        }
    }
     ...
     */
    public HUSP_SP_Contrast_MRSU(String input1, String input2, double minRatio, double minContrastUtility, String output) {
        this.input1 = input1;
        this.input2 = input2;
...
//            listIterator.remove();
    ...
    void firstUSpan() throws IOException {
        ...
        for (Map.Entry<Integer, Integer> entry : mapItemSwu1.entrySet()) {
            if (entry.getValue() < minUtility1) isRemove[entry.getKey()] = true;
        }
        MemoryLogger.getInstance().checkMemory();
    ...
    void firstConcatenation() throws IOException {
        ...
        for (Map.Entry<Integer, Integer> entry : mapItemTRSU1.entrySet()) {
            if (entry.getValue() >= minUtility1) {
                ...
                boolean isPattern = cUtil >= minContrastUtility && u1 >= minUtility1 && utilityUpper2 < minUtility2;
...
//                if(isPattern)
//                    patterns.add(prefix+ "u1 / u2: "+ String.format("%.5f", uRatio1 / uRatio2));
                if ( utilityUpper1 >= minUtility1 ) {
                    // call the function
    ...
    class UtilityAndUpper {
        ...
        public UtilityAndUpper(int a, int b){
            this.utility = a;
            this.upperBound = b;
        }
    }
    ...
    private UtilityAndUpper utilityCalculation(int addItem, ULinkList[] uLinkListDB, ArrayList<ProjectULinkList> newProjectULinkListDB) {
        ...
        return new UtilityAndUpper(sumUtility, upperBound);
    }
    ...
    protected void runHUSPspan(ArrayList<ProjectULinkList> projectULinkListDB1, ArrayList<ProjectULinkList> projectULinkListDB2) throws IOException {
        ...
        ArrayList<Integer> newRemoveItems = new ArrayList<>();
        ...
        removeItem(projectULinkListDB1);
        newRemoveItems.clear();
//        removeItem(projectULinkListDB2);

        // call the iConcatenation function
        HashMap<Integer, LastId> mapItemIConcatenationSwu1 = getMapItemIConcatenationSwu(projectULinkListDB1);
//        HashMap<Integer, LastId> mapItemIConcatenationSwu2 = getMapItemIConcatenationSwu(projectULinkListDB2);
//        HashSet<Integer> iExtensionItems1 = getIExtensionItems(projectULinkListDB1, isRemove1);
//        HashSet<Integer> iExtensionItems2 = getIExtensionItems(projectULinkListDB2, isRemove2);
        iConcatenation(projectULinkListDB1, projectULinkListDB2, mapItemIConcatenationSwu1);

        // check the memory usage
        MemoryLogger.getInstance().checkMemory();

        // call the sConcatenation function
        HashMap<Integer, LastId> mapItemSConcatenationSwu1 = getMapItemSConcatenationSwu(projectULinkListDB1);
//        HashMap<Integer, LastId> mapItemSConcatenationSwu2 = getMapItemSConcatenationSwu(projectULinkListDB2);
//        HashSet<Integer> sExtensionItems1 = getSExtensionItems(projectULinkListDB1, isRemove1);
//        HashSet<Integer> sExtensionItems2 = getSExtensionItems(projectULinkListDB2, isRemove2);
        sConcatenation(projectULinkListDB1, projectULinkListDB2, mapItemSConcatenationSwu1);
...
//        removeItem(projectULinkListDB2);
            ...
            for (UPosition uPosition : uPositions) {
                ...
                for (int index = uPosition.index() + 1; index < nextItemsetPos; ++index) {
                    ...
                    if (!isRemove[item]) {
                        ...
                        if (lastId == null) {
                            int trsu = get_TRSU_I(projectULinkList, index, uPosition);
                            mapItemIConcatenationSwu.put(item, new LastId(trsu, uLinkList, trsu));
                        } else {
                            // should not add sequence utility of same sequence more than once
                            ...
                            if (lastId.uLinkList != uLinkList) {
                                int trsu = get_TRSU_I(projectULinkList, index, uPosition);
                                lastId.swu += trsu;
                                lastId.maxTrsuInT = trsu;
                                lastId.uLinkList = uLinkList;
                            }else{
                                if(!firstPEU){
                                    int trsu = get_TRSU_I(projectULinkList, index, uPosition);
                                    if(trsu > lastId.maxTrsuInT){
                                        lastId.swu = lastId.swu - lastId.maxTrsuInT + trsu;
                                        lastId.maxTrsuInT = trsu;
                                    }
                                }
                            }
    ...
    private int get_TRSU_I(ProjectULinkList projectULinkList, int index, UPosition uPosition) {
//        ArrayList<UPosition> uPositions = projectULinkList.getUPositions();
        ULinkList uLinkList = projectULinkList.getULinkList();
//        int trsu = 0;
//        int pIndex = 0;
//        int mapIndex = 0;
        ...
        /*for (int i = 0; i < maxPEUPos; i++){
            UPosition position = uPositions.get(i);
        ...
        trsu = uLinkList.remainUtility(uPosition.index()) - uLinkList.remainUtility(index - 1);
        trsu = Math.max(trsu, 0);*/
        return uPosition.utility() + uLinkList.remainUtility(index - 1);
    }

    private int get_TRSU_S(ProjectULinkList projectULinkList, int index) {
        ArrayList<UPosition> uPositions = projectULinkList.getUPositions();
        ULinkList uLinkList = projectULinkList.getULinkList();
        int trsu = 0;
        int pIndex = 0;
        ...
        for (int i = 0; i < uPositions.size(); i++){
            ...
            if (position.index() < index) {
                if(position.utility() > maxU){
                    pIndex = position.index();
                    maxU = position.utility();
                }
            } else {
                break;
            }
        }
        return maxU + uLinkList.remainUtility(index - 1);
    }
...
//            } else {
            ...
            for (int index = addItemPos; index < uLinkList.length() && index != -1; ++index) {
                ...
                if (!isRemove[item]) {
                    ...
                    if (lastId == null) {
                        int trsu = get_TRSU_S(projectULinkList, index);
                        mapItemSConcatenationSwu.put(item, new LastId(trsu, uLinkList, trsu));
...
//        }
    ...
    protected void iConcatenation(ArrayList<ProjectULinkList> projectedDB1, ArrayList<ProjectULinkList> projectedDB2,
                                  HashMap<Integer, LastId> mapItemIConcatenationSwu1
                                  ) throws IOException {
...
//                    patterns.add(prefix+ "u1 / u2: "+ String.format("%.5f", uRatio1 / uRatio2));
    ...
    protected void sConcatenation(ArrayList<ProjectULinkList> projectedDB1, ArrayList<ProjectULinkList> projectedDB2,
                                  HashMap<Integer, LastId> mapItemSConcatenationSwu1
                                 ) throws IOException {
...
//                if (isPattern) {
//                    patterns.add(prefix+ "u1 / u2: "+ String.format("%.5f", cUtil));
//                }
                if ( utilityUpper1 >= minUtility1 ) {
                    // call the function
                    runHUSPspan(newProjectULinkListDB1, newProjectULinkListDB2);
...
//    }
    ...
    protected int getItemUpperBound(ProjectULinkList projectULinkList) {
        ULinkList uLinkList = projectULinkList.getULinkList();
        ArrayList<UPosition> uPositions = projectULinkList.getUPositions();
        //HashMap<Integer, Integer> indexPEUMap = new HashMap<>();
        int upperBound = 0;
        int count = 0;
        for (UPosition uPosition : uPositions) {
            count++;
            int localPeu = uPosition.utility() + uLinkList.remainUtility(uPosition.index());
            //indexPEUMap.put(count, localPeu);
        ...
        for (ProjectULinkList projectULinkList : projectULinkListDB) {
            ...
            for (int i = uPosition.index() + 1; i < uLinkList.length(); ++i) {
                ...
                if (!isRemove[item]) {
                    ...
                    if (lastId == null) {
                        mapItemExtensionUtility.put(item, new LastId(swuInT, uLinkList, swuInT));
                    } else {
                        if (lastId.uLinkList != uLinkList) {
                            lastId.swu += swuInT;
                            lastId.uLinkList = uLinkList;
...
//        System.out.println("min: " + minUtility1);

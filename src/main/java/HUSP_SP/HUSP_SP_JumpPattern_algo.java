...
public class HUSP_SP_JumpPattern_algo {
...
//    protected String output;
    ...
    HashSet<Integer> leftLowRightHigh = new HashSet<>();
    double u1u2 = 0.0;
     ...
     */
    public HUSP_SP_JumpPattern_algo(String input1, String input2, double minRatio, double minContrastUtility) {
        this.input1 = input1;
        this.input2 = input2;
...
//            listIterator.remove();
    ...
    void firstUSpan() throws IOException {
        ...
        HashMap<Integer, Integer> mapItemSwu2 = getMapItemSwu(uLinkListDB2);

        for (Map.Entry<Integer, Integer> entry : mapItemSwu1.entrySet()) {
            Integer key = entry.getKey();
            if (entry.getValue() < minUtility1 || mapItemSwu2.getOrDefault(key, 0) < minUtility2) {
                isRemove[entry.getKey()] = true;
            }
            if(entry.getValue() < minUtility1 && mapItemSwu2.getOrDefault(key, 0) >= minUtility2)
                leftLowRightHigh.add(key);
            if(mapItemSwu2.containsKey(key))
                mapItemSwu2.remove(key);
        }
        for (Map.Entry<Integer, Integer> entry : mapItemSwu2.entrySet()) {
            if(entry.getValue() < minUtility2)
                isRemove[entry.getKey()] = true;
        }
        MemoryLogger.getInstance().checkMemory();
...
//    }
    ...
    void firstConcatenation() throws IOException {
        ...
        for (Map.Entry<Integer, Integer> entry : mapItemTRSU1.entrySet()) {
            if (entry.getValue() >= minUtility1 && mapItemTRSU2.getOrDefault(entry.getKey(), 0) >= minUtility2) {
                ...
                int u1 = utilityAndUpper1.utility, utilityUpper1 = utilityAndUpper1.upperBound, minInstanceUtility1 = utilityAndUpper1.minInstanceUtility;
                int u2 = utilityAndUpper2.utility, utilityUpper2 = utilityAndUpper2.upperBound, minInstanceUtility2 = utilityAndUpper2.minInstanceUtility;
                double uRatio1 = (double) u1 / totalUtility1, uRatio2 = (double) u2/totalUtility2;
                double upRatio1 = (double) utilityUpper1/totalUtility1, upRatio2 = (double) minInstanceUtility2 /totalUtility2;
                double contrastUpper1 = upRatio2 == 0 ? minContrastUtility : upRatio1 / upRatio2;
                double contrastUpper2 = (double) utilityUpper2 / totalUtility2 / ((double) minInstanceUtility1 / totalUtility1);
                ...
                if(firstStep) {
                    u1u2 = cUtil;
                    secondStepConcat(newProjectULinkListDB1, newProjectULinkListDB2, minInstanceUtility1, minInstanceUtility2);
                }

                if (contrastUpper1 >= minContrastUtility  && contrastUpper2 >= minContrastUtility && utilityUpper1 >= minUtility1 && utilityUpper2 >= minUtility2) {
                    // call the function
                    runHUSPspan(newProjectULinkListDB1, newProjectULinkListDB2, minInstanceUtility1, minInstanceUtility2);
                }
    ...
    private void secondStepConcat(ArrayList<ProjectULinkList> projectULinkListDB1, ArrayList<ProjectULinkList> projectULinkListDB2, int minInstanceUtility1, int minInstanceUtility2) throws IOException {
        // call the iConcatenation function

        for (Integer integer : leftLowRightHigh) {
            isRemove[integer] = false;
        }
        removeItem(projectULinkListDB1);
        removeItem(projectULinkListDB2);

        iConcatenation_sec(projectULinkListDB1, projectULinkListDB2, minInstanceUtility1, minInstanceUtility2);

        // check the memory usage
        MemoryLogger.getInstance().checkMemory();

        // call the sConcatenation function
        sConcatenation_sec(projectULinkListDB1, projectULinkListDB2, minInstanceUtility1, minInstanceUtility2);

        // check the memory usage
        MemoryLogger.getInstance().checkMemory();

        for (Integer integer : leftLowRightHigh) {
            isRemove[integer] = true;
        }
        removeItem(projectULinkListDB1);
        removeItem(projectULinkListDB2);
    }
     ...
     *
     */
    protected void runHUSPspan(ArrayList<ProjectULinkList> projectULinkListDB1, ArrayList<ProjectULinkList> projectULinkListDB2, int minInstanceUtility1, int minInstanceUtility2) throws IOException {
        HashMap<Integer, LastId> mapItemExtensionUtility1 = getMapItemExtensionUtility(projectULinkListDB1);
        HashMap<Integer, LastId> mapItemExtensionUtility2 = getMapItemExtensionUtility(projectULinkListDB2);

        for (Map.Entry<Integer, LastId> entry : mapItemExtensionUtility1.entrySet()) {
            Integer key = entry.getKey();
            int value1 = entry.getValue().swu;
            LastId temp = mapItemExtensionUtility2.getOrDefault(key, null);
            int value2 = temp == null ? 0 : temp.swu;

            double urUpp1 =  (((double) value1 / totalUtility1) / ((double)minInstanceUtility2 / totalUtility2) );
            double urUpp2 = (((double) value2 / totalUtility2) / ((double)minInstanceUtility1 / totalUtility1) );

            if (value1 < minUtility1 || value2 < minUtility2 || Double.compare(urUpp1, minContrastUtility) < 0 || Double.compare(urUpp2, minContrastUtility) < 0 ){
//            if (value1 < minUtility1 || value2 < minUtility2  ){
...
//            if((value1 < minUtility1 && value2 >= minUtility2) )
        ...
        for (Map.Entry<Integer, LastId> entry : mapItemExtensionUtility2.entrySet()) {
            Integer key = entry.getKey();
            int value1 = entry.getValue().swu;
            if(value1 < minUtility2)
                isRemove[key] = true;
        }
        ...
        // call the iConcatenation function
        iConcatenation(projectULinkListDB1, projectULinkListDB2, minInstanceUtility1, minInstanceUtility2);

        // check the memory usage
        MemoryLogger.getInstance().checkMemory();

        // call the sConcatenation function
        sConcatenation(projectULinkListDB1, projectULinkListDB2, minInstanceUtility1, minInstanceUtility2);
...
//            int value1 = entry.getValue().swu;
//            LastId temp = mapItemExtensionUtility2.getOrDefault(key, null);
//            int value2 = temp == null ? -1 : temp.swu;
//            if (value1 < minUtility1 || value2 < minUtility2) {
//                isRemove[key] = false;
//            }
//            if(value1 < minUtility1 && value2 >= minUtility2)
//                leftLowRightHigh.remove(key);
//        }
//
//        for (Map.Entry<Integer, LastId> entry : mapItemExtensionUtility2.entrySet()) {
//            Integer key = entry.getKey();
...
//        }
     ...
     *
     */
    protected void iConcatenation(ArrayList<ProjectULinkList> projectedDB1, ArrayList<ProjectULinkList> projectedDB2,
                                  int minInstanceUtility1_O, int minInstanceUtility2_O) throws IOException {
        ...
        for (Map.Entry<Integer, LastId> entry : mapItemIConcatenationSwu1.entrySet()) {
            ...
            double contrastUpp1 = ((double) swu1 / totalUtility1) / ((double) minInstanceUtility2_O / totalUtility2);
            double contrastUpp2 = ((double) swu2 / totalUtility2) / ((double) minInstanceUtility1_O / totalUtility1);
            if (swu1 >= minUtility1 && swu2 >= minUtility2 && contrastUpp1 >= minContrastUtility && contrastUpp2 >= minContrastUtility) {
                candidateNum += 1;
                int addItem = entry.getKey();
    ...
    protected void iConcatenation_sec(ArrayList<ProjectULinkList> projectedDB1, ArrayList<ProjectULinkList> projectedDB2,
                                  int minInstanceUtility1_O, int minInstanceUtility2_O) throws IOException {
        HashMap<Integer, LastId> mapItemIConcatenationSwu2 = getMapItemIConcatenationSwu(projectedDB2);
        for (Map.Entry<Integer, LastId> entry : mapItemIConcatenationSwu2.entrySet()) {
            int swu1 = entry.getValue().swu;
            double contrastUpp = ((double) swu1 / totalUtility2) / ((double) minInstanceUtility1_O / totalUtility1);
//            if (swu1 >= minUtility2 && contrastUpp >= minContrastUtility ) {
            if (swu1 >= minUtility2 ) {
//                candidateNum += 1;
                halfQualifiedNum++;
                int addItem = entry.getKey();
...
//                    patterns.add(prefix + "u1 / u2: "+ String.format("%.5f", u1u2) + "u2 / u1: "+ String.format("%.5f", cUtil));
     ...
     *
     */
    protected void sConcatenation(ArrayList<ProjectULinkList> projectedDB1, ArrayList<ProjectULinkList> projectedDB2,
    ...
    protected void sConcatenation_sec(ArrayList<ProjectULinkList> projectedDB1, ArrayList<ProjectULinkList> projectedDB2,
                                  int minInstanceUtility1_O, int minInstanceUtility2_O) throws IOException {
        HashMap<Integer, LastId> mapItemSConcatenationSwu2 = getMapItemSConcatenationSwu(projectedDB2);
...
//    }
    ...
    public void printStatistics() throws IOException {
        System.out.println("=============  JUCP-SP - STATS ============");
...
//        System.out.println("minContrastUtility: " + minContrastUtility);
        ...
        System.out.println("minRatio: " + minContrastUtility);
